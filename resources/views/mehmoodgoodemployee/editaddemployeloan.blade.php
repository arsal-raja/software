...
<h4>Update Employee Loan/Loan Return</h4>
<div class="text-left">
...
<option value="{{$value->branch_id}}" data-id="{{ \Crypt::encrypt($value->id) }}" {{$EmployeeLoan[0]->Bankid==$value->id?"Selected":""}}>{{$value->branch->bank->name}}/{{$value->branch->name}}</option>
@if(!empty($value->id))
