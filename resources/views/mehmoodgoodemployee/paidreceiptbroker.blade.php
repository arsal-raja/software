...
<select value="{{ old('customer_id') }}" onchange="getbrokerbalance(this)"   name="customer_id" id="customer_id" class="form-control form-control-round mdb-select md-form colorful-select dropdown-primary" >
  <option value="">Select Broker Name</option>
   ...
   @endforeach
                               ...
                               <div class="col-span-6 mt-2">
                                    <label for="regular-form-6" class="form-label">Balance Type</label>
<select value="{{ old('customer_id') }}" onchange="getcustomerbalance(this)"   name="balancetype" id="balancetype" class="form-control form-control-round mdb-select md-form colorful-select dropdown-primary" >
  ...
  <option value="Receivable">Receivable</option>                             
                               ...
                               <div class="col-span-6 mt-2">
                                    <label for="regular-form-6" class="form-label">Payble</label>
...
<option value="Petty Cash">Petty Cash</option>
                               ...
                               <div class="col-span-6 mt-2">
                                    <label for="regular-form-6" class="form-label">Receivable</label>
...
@foreach($bank_accounts as $value)
 ...
 <input value="{{ old('description') }}"  type="text" id="description" name="description"  class="form-control form-control-round" placeholder="Enter Description">
                              </div>
                              <div id="dynamics">
 <table class="table table-bordered" id="dynamic_field" >
                              ...
                              <button id="savebutton" type="submit" class="btn btn-primary mt-5">Save</button>
                              ...
                              <table id="example" class="display nowrap" style="width:100%">
                                 ...
                                 <tbody>
                                   @if(isset($records))
                                        ...
                                        <td>{{$key+1}}</td>
                                        <td>{{$value->date ??''}}</td>
                                        <td>{{$value->getbroker->broker_name ??''}}</td>
                                        <td>{{$value->amount ??''}}</td>
                                        <td>{{$value->payment_type ??''}}</td>
                                        <td>
                                         
                                         <a href="{{url('/mehmoodgoodemployee/viewpaidreceiptbroker/'.$value->id)}}"><i class="fa fa-eye"></i></a>&nbsp;|&nbsp;
                                          
                                            <a id="deletebrokerpaidreceipt" data-deleteId="{{$value->id}}"><i class="fa fa-trash delete"></i></a>     
                                           </td>
                                        </tr>
                                        @endforeach
                                        @endif
...
<!--  <h4 id="balance"></h4>-->
<!--</div>  -->
<!--<input type="hidden" id="checkbalance">-->
<!--<div class="card-header">-->
<!--<h4>Paid Receipt Broker</h4>-->
  ...
  function getpaymenttype(t) 
  {
    ...
    if(type=="Bank")
    {
      $("#bank").show();
       ...
       $("#bank").hide();
       $.ajax({
      url:'getpettycashamount',
      type:'GET',
      async: false,
      data: {  },
      success:function(data){
       $('#balance').text("Balance: "+data);
       $('#checkbalance').val(data);
         }
        });
      } 
...
//on getting customer balance if to pay is selected
 function getbrokerbalance(t) 
  {
    var broker = t.value;
    
    $.ajax({
     url:'getbrokerbalance',
     type:'GET',
     async: false,
     data: { broker:broker },
     success:function(data){

       $('#prevbalance').val(data.payable);
...
{
  let previous = $('#checkbalance').val();
  let current = $('#amount').val();
  let balancetype = $('#balancetype').val(); 
  if(balancetype=="Payable"){
     ...
     $('#amount').val(previous);
      ...
      document.getElementById("savebutton").disabled = false; 
      Swal.fire({
      title: 'Your enter amount is greater than total amount maximum amount allowed is this',
      showClass: {
...
$(document).on("click","#deletebrokerpaidreceipt", function(e){ //user click on remove text
  ...
  Swal.fire({
    ...
    if (result.value) 
    {
      $.ajax({
        type: 'get',
        url: '{{url('deletebrokerpaidreceipt')}}',
        data: data,
        date: 'json',
        headers: {
