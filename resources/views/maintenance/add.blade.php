 ...
 @include('includes.mobile-nav')
      <div class="flex">
         @include('includes.side-nav')
            ...
            <div class="top-bar">
               <!-- BEGIN: Breadcrumb -->
               <div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="">Application</a> <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Maintainance</a> </div>
               <!-- END: Breadcrumb -->
               <!-- BEGIN: Search -->
            ...
            <!-- END: Top Bar -->
               @if(!empty($edit_maintenances))
               <form method="post" action="{{ route('save-maintenance-edit-process') }}">
                  @endif
                  
                  @if(empty($edit_maintenances))
               <form method="post" action="{{ route('head-maintenance-process') }}">
                  @endif
            @csrf
            <div class="grid">
               <div class="intro-y flex items-center mt-12">
                  <h2 class="text-lg font-medium mr-auto">
                     Maintenance Head Form
                  </h2>
               </div>
                           ...
                           <div class="preview">
                              ...
                              <div class="grid grid-cols-12 gap-2">
                                  <div class="col-span-6 mt-2">
                                     <label for="regular-form-4" class="form-label">Head Name</label>
                                     @if(!empty($edit_maintenances))
                                     <input  type="hidden" value="{{$edit_maintenances->id}}" name="id">
                                     @endif
                                     <input required id="regular-form-4" value="@if(!empty($edit_maintenances)) {{$edit_maintenances->headname}} @endif" type="text" class="form-control" placeholder="Enter Head Name" name="headname">
                                  </div>
                                  <div class="col-span-6 mt-2">
                                    <label for="regular-form-4" class="form-label">Detail</label>
                                    <input id="regular-form-4" type="text" class="form-control" value="@if(!empty($edit_maintenances)) {{$edit_maintenances->details}} @endif" placeholder="Enter Detail" name="details">
                                  </div>
                              </div>
                               ...
                               <div class="grid grid-cols-12 gap-2">
                                  <div class="col-span-6 mt-2">
                                     ...
                                     <input id="txtBox" name="maintaindate" value=" {{$edit_maintenances->maintaindate}}" type="text" class="form-control">


                                     @endif
                                     @if(empty($edit_maintenances))
                                     <input id="txtBox" name="maintaindate" type="date" class="form-control">
                                       @endif
                                    </div>
                                  <div class="col-span-6 mt-2">
                                    <label for="regular-form-4" class="form-label">Category</label>
                                    <input id="regular-form-4" type="text" class="form-control" value="@if(!empty($edit_maintenances)) {{$edit_maintenances->category}} @endif" placeholder="Enter Category" name="category">
                                  </div>
                              </div>
                           ...
                           <div class="preview">
                                 ...
                                 <tbody>
                                       ...
                                       @foreach($maintenances as $row)
                                             ...
                                             <td>{{$row->category}}</td>
                                            <td>{{$row->details}}</td>
                                            <td><a href="miantenance-edit/{{$row->id}}">Edit</a> | 
                                            <a onclick="return confirm('Are you sure you want to delete {{$row->maintaindate}} ?')" href="miantenance-del/{{$row->id}}">Delete</a></td>
</tr>
