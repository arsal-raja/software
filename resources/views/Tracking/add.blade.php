             ...
             <form method="post" action="{{route('track.challan')}}">   
               ...
               <div class="grid grid-cols-12 gap-2 mt-2 positon-relative senderForm">
                     <div class="col-span-6 mt-2">
                            <label for="regular-form-6" class="form-label">Challan No</label>
                            <select type="text" id="regular-form-6" class="form-control"  placeholder="Enter Address" name="id">
                         <option>Select Challan</option>
                         @foreach($challant as $value)

                         <option value="{{$value->vehicle_id}}" @if(!empty($query)) @if($value->id == $query->id) {{'selected'}}  @endif @endif>{{$value->vehicle_id}}</option>
                         @endforeach
                         </select>
                        ...
                        <div id="input" class="p-5">
                            <form method="post" @if(!empty($query)) action="{{route('edit-tracking-process')}}" @endif @if(empty($query)) action="{{route('add-tracking-process')}}" @endif>
               @csrf
                           <div class="preview">
                               <div class="grid grid-cols-12 gap-6 mt-5">
                              <div class="col-span-6 mt-2">
                              ...
                              <!--<div class="col-span-6 mt-2">
                                    <label for="regular-form-6" class="form-label">Truck No</label>
                                    <select type="text" id="regular-form-6" class="form-control"  placeholder="Enter Address" name="truckno">
                                 <option>Please select Truck</option>
                                 @foreach($truck as $value)
                                 <option value="{{$value->id}}" @if(!empty($query)) @if($value->id == $query->Tracking_Truck_no) {{'selected'}}  @endif @endif>{{$value->vehicleno}}</option>   
                                 @endforeach
                                 </select>
                                 </div>-->
                             ...
                             <option>Select Challan</option>
                             @foreach($challant as $value)
                             <option value="{{$value->vehicle_id}}" @if(!empty($query)) @if($value->id == $query->vehicle_id) {{'selected'}}  @endif @endif>{{$value->vehicle_id}}</option>
                             @endforeach
                             </select>
                             </div>
                              <div class="col-span-6 mt-2">
