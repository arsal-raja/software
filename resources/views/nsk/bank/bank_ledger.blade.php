 ...
 <div class="flex">
       @include('includes.side-nav')
         <!-- BEGIN: Content -->
            ...
            <div class="top-bar">
               <!-- BEGIN: Breadcrumb -->
               <div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="">Application</a> <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Bank Ledger</a> </div>
               <!-- END: Breadcrumb -->
               <!-- BEGIN: Search -->
...
<div class="right_col" role="main" style="min-height: 1704px;">
          ...
          <div class="clearfix"></div>
                <form method="POST" name="ledger_report" align="center" action="{{ url('ledger_report') }}"  class="demo-form2"  >
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <div class="col-md-3 col-xs-12 col-lg-3">
                <meta name="csrf-token" content="{{csrf_token()}}">
                    <div class="form-group">
                        <label class="form-label">Bank Name<span class="red-color">*</span></label>
                        <div class="controls">
                            <select id="bank_id" name="bank_id" class="form-control" >
                            <option value="">All Bank</option>
                            <?php
                            if(isset($banks_records) && sizeof($banks_records) > 0){
                                foreach($banks_records as $rec){
                ...
                <div class="col-md-3">
                    ...
                    <div class="controls">
                    <input type="date" name="from_date"  id="datepicker1" class="form-control single_cal2"  required="required"  placeholder="From Date" >
                    </div>
                </div>
                <div class="col-md-3">
                    <label class="form-label">TO DATE </label>
                    <div class="controls">
                    <input type="date" name="to_date"  id="datepicker2" class="form-control single_cal2"  required="required" placeholder="To Date" >
                    </div>
                </div>
                    <div class="col-md-2">
                    <button type="submit" class="signbuttons btn btn-primary button_call" name="ledger_report" class="ledger_report" id="btn_submit" style="margin-top:23px;">Generate Report </button>
                    </div>
                </form>
                <hr />
                <!-- /Date to Date Report -->
                    <div class="container">
                    <h2>Bank Ledger Records </h2>
                    <table id="datatable-buttons" class="table table-striped table-bordered">
                        <thead>
                        <tr>
                            <th>SN#</th>
		                        <th>Date</th>
                            <th>Particular/ Description | Bank | Cheque</th>
                            <th>Deposit Slip Number</th>
                            <th>Payment</th>
                            <th>Reciept</th>
                            <th>Balance</th>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td><?php
                							if($rec->bank_ledger_credit == '' || $rec->bank_ledger_credit == NULL){
                								$date_format = date('d-m-Y',strtotime($rec->bank_ledger_date));
                							}else{
                								$date_format = date('M-Y',strtotime($rec->bank_ledger_date));
                							}
                							echo $date_format;
                						?></td>
                            <td>{{ $rec->CustomerName }}
                              @foreach($banks_records as $row)
                              <?php if($row->PKBankID == $rec->FKBankID){echo '| '.$row->BankName;}else{} ?> {{ '| '.$rec->ChequeNumber }}
                              @endforeach
                            </td>
