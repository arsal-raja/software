               ...
               <div class="grid grid-cols-12 gap-6 mt-5">
                  ...
                  <div class="intro-y col-span-12 lg:col-span-12">
                     ...
                     <div class="intro-y box">
                        ...
                        <form method="post"  autocomplete="off"  action="{{Route('save-Driver-assign')}}">
                           @csrf    
                        <div id="input" class="p-5">
                        <div class="col-span-4">
                              <div class="mt-2">
                                 <label for="regular-form-2"  class="form-label">Vechicle NO</label>
                                 <select id="regular-form-2" name="VechicleID" class="form-control">
                                   <option> Select Vechicle NO </option>
                                 @foreach($vehicle as $vRow)
                                    <option @if(!empty($select)) @if($vRow->id == $select->VechicleNO) {{'selected'}} @endif  @endif value="{{$vRow->id}}">{{$vRow->vehicleno}}</option>
                                    @endforeach
                                </select>
                              </div>
...
@if(!empty($select)) 
<input type="hidden" name="assignid" value="{{$select->id}}">
@endif

   <div class="col-span-4">

      <label for="regular-form-4" class="form-label">Driver Name</label>
      <select id="regular-form-2" name="DriverName[]" class="form-control">
                                      <option> Select Driver Name </option>
                                      @foreach($driver as $dRow)
                                    <option @if(!empty($select)) @if($dRow->id == $select->DriverID) {{'selected'}} @endif  @endif value="{{$dRow->id}}">{{$dRow->DriverName}}</option>
                                    @endforeach
                                    </select>
   </div>
   <div class="col-span-4">
   <label for="regular-form-6" class="form-label">Degisnation</label>

<select id="regular-form-2" name="designation[]" class="form-control">
<option> Select Designation </option>
    <option @if(!empty($select)) @if($select->Designation == 'Driver') {{'selected'}} @endif  @endif   >Driver</option>
     <option @if(!empty($select)) @if($select->Designation == 'Conductor') {{'selected'}} @endif  @endif>Conductor</option>
     <option @if(!empty($select)) @if($select->Designation == 'Cleander') {{'selected'}} @endif  @endif>Cleander</option>
  </select>
 <!--<button type="button" class="btn btn-primary mt-5 addasign">Add More</button>-->
...
<td>
<?php
$Vname = DB::table('vehicles')->where('id',$row->VechicleNO)->first();
$dname = DB::table('driver_name')->where('id',$row->DriverID)->first(); 
?>
{{$Vname->vehicleno}}
</td>
<td>
...
<td>
<a href="{{route('save-edit-driver-assign',['id'=>$row->id])}}"> Edit  </a>  |
<a href="{{route('delete-Driver-assign',['id'=>$row->id])}}"> Delete </a> 
