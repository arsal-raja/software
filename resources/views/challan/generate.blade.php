...
// $sql = "SELECT * FROM `now_challan` WHERE challanid = '".$id."'";
// $result = $conn->query($sql);
// $rows = $result->fetch_assoc();
// $challanid = $rows["challanid"];
// $challandate = $rows["challandate"];
// $date = $rows["date"];
// $challandriver = $rows["challandriver"];
// $loadername = $rows["loadername"];
// $dispatchtime = $rows["dispatchtime"];
// $containerno = $rows["containerno"];
// $containerseal = $rows["containerseal"];

// $truckno = $rows["truckno"];
// $challanfrom = $rows["challanfrom"];
// $challanto = $rows["challanto"];
            ...
            <div class="col-sm-12">
                <div class="panel panel-bd">
                    ...
                    <div class="col-sm-8 mid-level-text" style="text-align: center;font-weight: 500;display:inline-block;vertical-align: top;width: 500px;">
                        <img src="https://asaanapps.com/Noshera/src/images/logo.png" alt="" style="width: 100%;max-width: 150px;margin: 0 auto;margin-bottom: 10px;">
                    ...
                    <div class="col-sm-2" style="width: 130px;display:inline-block;vertical-align: top;float: right;">
                    		<table>
                    			<tr><td>No.  <br>{{$generate_challan->id }}</td></tr>
                    			<tr><td>Date:  {{$generate_challan->date}}</td></tr>

                                <?php
                                        $vehicle =  DB::table('now_vehicles')
                                       ->where('id',$generate_challan->vehicle_id)
                                        ->first();
                                    ?>
                    			...
                    			<tr><td>Truck No:{{$vehicle->vno}}</td></tr>
                ...
                <div class="printViewHeader_Second">
                    <table>
                        <tr>
                            <td><b>Chalan No</b>{{$generate_challan->id}}</td>
                            <td><b>Driver Name</b>{{$generate_challan->driver}}</td>
                        </tr>
                        <tr>
                        ...
                        <?php
                                ...
                                ->where('id',$generate_challan->from_station)
                                ->first();

                                $to_station = DB::table('now_station')
                                ->where('id',$generate_challan->to_station)
                                ->first();
...
?>
                            ...
                            <td><b style="width: 230px;">Goods booked FROM {{$form_station->name}} TO {{$to_station->name}}</td>
                            <td><b>Broker Name</b> {{$generate_challan->broker}}</td>
                        </tr>
                        <tr>
                            <td><b>Date</b>  {{$generate_challan->date}}</td>
                            <td><b>Mobile</b> {{$generate_challan->mobile_no}}</td>
                        </tr>
                        <tr>
                            <td><b>Truck No</b> {{$vehicle->vno}}</td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="printViewHeader_table">
                    <table>
                        <tr>
                         ...
                         <?php
                // $sql = "SELECT * FROM `now_challanlist` WHERE challanid = '".$challanid."'";
                // $result = $conn->query($sql);

                // if ($result->num_rows > 0) {
                //   while($row = $result->fetch_assoc()) {

                // $sqls = "SELECT * FROM `now_builty` WHERE builtyid = '".$row["trno"]."'";
                ...
                //  $sqls = "SELECT SUM(weight) AS totalweight FROM `now_builtyitems` WHERE builtyid = '".$row["trno"]."'";
                // $results = $conn->query($sqls);
                // while($rows = $results->fetch_assoc()) {
                //    $totalweight = $rows['totalweight'];
                //    $sendername = $rows['sendername'];
                // }
